...
export const decisionLedger = mysqlTable("decision_ledger", {
  ...
  // ── Phase 7: Intelligence Validation Center fields ──────────────────────────
  /** Sector classification e.g. Technology, Healthcare, Financials, Energy, Crypto, ETF, Macro */
  sector:           varchar("sector", { length: 40 }),
  /** Recommendation type e.g. Accumulate, Reduce, Avoid, Bullish, Bearish, Day Trade, Swing Trade */
  recommendationType: varchar("recommendationType", { length: 40 }),
  /** Engine that produced this recommendation e.g. Ask Intelligence, Signals, Day Trade Intelligence */
  engineSource:     varchar("engineSource", { length: 50 }),
  /** Actual return % from entry to resolution (positive = gain, negative = loss) */
  returnPct:        double("returnPct"),
  /** Maximum drawdown % experienced during the holding period */
  drawdownPct:      double("drawdownPct"),
  /** Hours from recommendation to target hit (null if target not hit) */
  ...
  engineIdx:    index("decision_ledger_engineSource_idx").on(t.engineSource),
  sectorIdx:    index("decision_ledger_sector_idx").on(t.sector),
}));
...
// ─────────────────────────────────────────────────────────────────────────────
// Improvement Lessons
// LLM-extracted lessons from resolved Decision Ledger entries.
// One row per lesson extracted from a resolved recommendation.
// ─────────────────────────────────────────────────────────────────────────────
export const improvementLessons = mysqlTable("improvement_lessons", {
  id:             bigint("id", { mode: "number", unsigned: true }).autoincrement().primaryKey(),
  /** The user whose recommendation produced this lesson */
  userId:         int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  /** FK to the resolved decision_ledger entry */
  ledgerEntryId:  int("ledgerEntryId").notNull(),
  /** Ticker symbol (null for macro) */
  ticker:         varchar("ticker", { length: 20 }),
  /** Asset type */
  assetType:      varchar("assetType", { length: 20 }),
  /** The original verdict */
  verdict:        varchar("verdict", { length: 32 }),
  /** The final outcome */
  outcome:        varchar("outcome", { length: 30 }),
  /** The extracted lesson text (1-3 sentences, actionable) */
  lessonText:     text("lessonText").notNull(),
  /** Pattern tag for grouping: Overconfidence | Macro Blind Spot | Timing Error | Sector Rotation | Regime Mismatch | Correct Thesis | Partial Signal | etc. */
  patternTag:     varchar("patternTag", { length: 60 }),
  /** Confidence score of the lesson extraction 0-100 */
  confidence:     int("confidence"),
  /** Engine that produced the original recommendation */
  engineSource:   varchar("engineSource", { length: 50 }),
  /** Market regime at the time of the recommendation */
  regimeAtTime:   varchar("regimeAtTime", { length: 80 }),
  createdAt:      timestamp("createdAt").defaultNow().notNull(),
}, (t) => ({
  userIdIdx:       index("improvement_lessons_userId_idx").on(t.userId),
  ledgerEntryIdx:  index("improvement_lessons_ledgerEntry_idx").on(t.ledgerEntryId),
  patternTagIdx:   index("improvement_lessons_patternTag_idx").on(t.patternTag),
  engineIdx:       index("improvement_lessons_engineSource_idx").on(t.engineSource),
}));
export type ImprovementLesson = typeof improvementLessons.$inferSelect;
export type InsertImprovementLesson = typeof improvementLessons.$inferInsert;

// ─────────────────────────────────────────────────────────────────────────────
// AI Improvement Reports
// Weekly LLM-generated reports synthesizing patterns from improvement_lessons.
// One row per week. Generated by the weekly heartbeat cron.
// ─────────────────────────────────────────────────────────────────────────────
export const aiImprovementReports = mysqlTable("ai_improvement_reports", {
  id:               bigint("id", { mode: "number", unsigned: true }).autoincrement().primaryKey(),
  /** ISO week string YYYY-Www e.g. 2025-W23 */
  weekOf:           varchar("weekOf", { length: 10 }).notNull().unique(),
  /** Full LLM-generated report text (Markdown) */
  reportText:       text("reportText").notNull(),
  /** Total recommendations analyzed this week */
  totalAnalyzed:    int("totalAnalyzed").default(0).notNull(),
  /** Count of correct outcomes */
  correctCount:     int("correctCount").default(0).notNull(),
  /** Count of incorrect outcomes */
  incorrectCount:   int("incorrectCount").default(0).notNull(),
  /** Count of partially correct outcomes */
  partialCount:     int("partialCount").default(0).notNull(),
  /** Count of still_active outcomes */
  activeCount:      int("activeCount").default(0).notNull(),
  /** JSON array of top recurring pattern tags */
  topPatterns:      text("topPatterns"),
  /** JSON array of identified weaknesses */
  weaknesses:       text("weaknesses"),
  /** JSON array of actionable recommendations for improving the engines */
  recommendations:  text("recommendations"),
  /** Overall accuracy rate for the week (0-100) */
  accuracyRate:     double("accuracyRate"),
  /** When this report was generated */
  generatedAt:      timestamp("generatedAt").defaultNow().notNull(),
  createdAt:        timestamp("createdAt").defaultNow().notNull(),
}, (t) => ({
  weekOfIdx: index("ai_improvement_reports_weekOf_idx").on(t.weekOf),
}));
