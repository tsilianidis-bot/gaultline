...
export function buildAtomicIntelligenceStateManifest({ pressure, seismograph, generatedAt = new Date().toISOString(), persistedHooks }: { pressure: FaultlinePressureOutput; seismograph: SeismographOutput | null; generatedAt?: string; persistedHooks?: { systemicRegime?: unknown; signalConvergence?: unknown } }): { manifest: AtomicIntelligenceStateManifest; claims: GovernedClaimRecord[]; inputQuality: LiveInputQualityManifestEntry[] } {
  ...
  const core = {
    ...
    engineDirections,
    domainValues: {
      activeContributors: seismograph?.activeContributors ?? [],
      evidenceConsensus: seismograph?.evidenceConsensus ?? "unavailable",
      providerProvenance: seismograph?.providerProvenance ?? null,
      // Additive persisted snapshots. NEVER enter engineValues / Pressure weights.
      ...(persistedHooks?.systemicRegime ? { systemicRegime: persistedHooks.systemicRegime } : {}),
      ...(persistedHooks?.signalConvergence ? { signalConvergence: persistedHooks.signalConvergence } : {}),
    },
