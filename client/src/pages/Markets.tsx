...
function generateLocalInterpretation(
  ...
  const usRead = usEquitiesReadSentence(s.usEquities);
  ...
  if (s.europe === "negative" || s.asia === "negative") {
    const region = s.europe === "negative" ? "European" : "Asian";
    parts.push(`International markets are showing weakness — ${region} indices are broadly lower, suggesting the risk-off tone extends beyond U.S. borders.`);
  ...
  if (parts.length === 0) return "Market observations are unavailable. Interpretation is withheld.";
  return parts.join(" ");
}
...
function BreadthRow({
  ...
  const pct = unavailable ? 0 : Math.min(100, (numeric / max) * 100);
  return (
    <div style={{ marginBottom: "10px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
        <span style={{ fontFamily: SANS, fontSize: "11px", color: TEXT }}>{label}</span>
        <span style={{ fontFamily: MONO, fontSize: "11px", color: unavailable ? MUTED : color }}>
          {unavailable ? (display ?? MARKETS_UNAVAILABLE_LABEL) : `${numeric.toFixed(1)}%`}
        </span>
      </div>
      <div style={{ height: "3px", background: "rgba(255,255,255,0.06)", borderRadius: "2px" }}>
        <div style={{ height: "100%", width: `${pct}%`, background: unavailable ? MUTED : color, borderRadius: "2px", transition: "width 0.5s ease" }} />
      </div>
    </div>
  );
}
...
export default function Markets() {
  ...
  const advancing = usIndicesAdvancing(usItems);
  const spread = rutVersusSpxSpread(
    usItems.find(i => i.symbol === "^RUT"),
    usItems.find(i => i.symbol === "^GSPC"),
  );
  const vixPrice = vixItems.find(i => i.symbol === "^VIX")?.price ?? null;
  const vixItem = vixItems.find(i => i.symbol === "^VIX");
  const vixUsable =
    vixPrice !== null &&
    Number.isFinite(vixPrice) &&
    vixItem?.freshnessState !== "STALE" &&
    vixItem?.freshnessState !== "UNAVAILABLE";
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              ...
              <div style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: "8px", padding: "16px" }}>
                ...
                <BreadthRow
                  ...
                  display={advancing.label}
                />
                {vixUsable && vixPrice !== null && (
                  <BreadthRow
                ...
                <div style={{ marginTop: "10px", padding: "8px 10px", background: "rgba(255,255,255,0.02)", borderRadius: "4px" }}>
                  ...
                  <div style={{
                    ...
                    RUT vs SPX: {spread.kind === "unavailable" ? MARKETS_UNAVAILABLE_LABEL : `${spread.label} spread`}
                    {spread.kind === "spread" && !spread.live ? " · DELAYED" : ""}
                  </div>
                  {spread.commentary && (
                ...
                {[
                  {
                    ...
                    value: crossAssetRelationship({
                      left: snapshot?.summary.dollar,
                      right: snapshot?.summary.usEquities,
                      fallback: "Neutral",
                      isDivergence: snapshot?.summary.dollar === "strengthening" && snapshot?.summary.usEquities === "risk-on",
                      isAligned: snapshot?.summary.dollar === "weakening" && snapshot?.summary.usEquities === "risk-on",
                      divergence: "Divergence — watch for reversal",
                      aligned: "Aligned — dollar weakness supporting equities",
                    }),
                  ...
                  },
                  {
                    ...
                    value: crossAssetRelationship({
                      left: snapshot?.summary.rates,
                      right: snapshot?.summary.usEquities,
                      fallback: "Neutral",
                      isDivergence: snapshot?.summary.rates === "rising" && snapshot?.summary.usEquities === "risk-on",
                      isAligned: snapshot?.summary.rates === "falling" && snapshot?.summary.usEquities === "risk-on",
                      divergence: "Tension — rising yields pressuring valuations",
                      aligned: "Supportive — falling yields tailwind for equities",
                    }),
                  ...
                  },
                  {
                    ...
                    value: crossAssetRelationship({
                      left: snapshot?.summary.crypto,
                      right: snapshot?.summary.usEquities,
                      fallback: "Mixed",
                      isAligned: snapshot?.summary.crypto === "positive" && snapshot?.summary.usEquities === "risk-on",
                      isDivergence: snapshot?.summary.crypto === "negative" && snapshot?.summary.usEquities === "risk-on",
                      aligned: "Aligned — risk appetite broad",
                      divergence: "Divergence — crypto not confirming equity strength",
                    }),
                    color: TEXT,
                  },
                ].map(row => (
                  <div key={row.label} style={{ marginBottom: "12px" }}>
                    ...
                    <div style={{ fontFamily: SANS, fontSize: "12px", color: row.value === MARKETS_UNAVAILABLE_LABEL ? MUTED : row.color, marginTop: "2px" }}>{row.value}</div>
                  </div>
                ))}
              </div>
            </div>
